...
const loadPlaylistTracks = ({ tracks }) => {
    ...
    // Ensure that the track object exists and has a preview_url before filtering
    for (let trackItem of tracks.items.filter(item => item.track && item.track.preview_url)) {
...
//     // Add the volume icon update code here
...
//     updateVolumeIcon();  // Initialize the correct icon on page load
...
//     window.addEventListener("popstate", (event) => {
//         loadSection(event.state);
//     // Adding an event listener for the "Home" button click
